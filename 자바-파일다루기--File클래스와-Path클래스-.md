...
이 외에도 자바의 `Path` 클래스, `Paths` 클래스를 활용하면 훨씬 유연한 파일 처리를 할 수 있다.

<br>

`Path` 클래스는 이런 문제점을 해결해주는 메서드들과 파일경로를 유연하게 처리해줄 수 있는 유용한 메서드들을 많이 갖추고 있다. 또한 `File` 클래스를 `Path` 클래스 내에서 혼합해서 사용하는 것 역시 가능하다.<br>
